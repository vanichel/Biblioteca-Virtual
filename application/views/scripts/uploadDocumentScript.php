...
function uploadFile(){
	var file = _("fileUploadDocument").files[0];
    var file2 = _("fileUploadImage").files[0];
	var formdata = new FormData($('#uploader')[0]);
    if (comprobarDatos()){
	formdata.append("fileUploadDocument", file);
	...
	ajax.open("POST", "<?=base_url()?>upload/documento");
	ajax.send(formdata);
    }
    else{
        finish();
...
function completeHandler(event){
    ...
    window.location.href = "<?=base_url()?>" +"admin/finalizado/"+id+"/libro";
}
